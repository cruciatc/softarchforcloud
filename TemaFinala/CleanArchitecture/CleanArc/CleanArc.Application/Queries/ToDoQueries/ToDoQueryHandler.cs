...
namespace CleanArc.Application.Queries.ActivityQueries
{
    public class ToDoQueryHandler : IRequestHandler<GetAllToDoQuery, GetAllToDoQueryResponse>,
        ...
        IRequestHandler<GetAllToDoByNameQuery, GetAllToDoByNameQueryResponse> //***Cornel******
    {
        ...
        public async Task<GetAllToDoByNameQueryResponse> Handle(GetAllToDoByNameQuery request, CancellationToken cancellationToken)
        {
            var response = new GetAllToDoByNameQueryResponse();
            var todos = _repository.GetEntities<ToDo>().Where(x => x.Name.Contains(request.ToDoName)  && x.UserId == request.User.UserId).ToList();
            var toDosResponse = todos.Select(x => new ToDoProjection { Id = x.Id, Name = x.Name, Description = x.Description }).ToList();
            response.ToDoModels = toDosResponse;
            return response;
        }
